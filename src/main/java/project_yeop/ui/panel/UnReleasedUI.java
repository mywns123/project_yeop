...
public class UnReleasedUI extends JPanel implements ActionListener {
	...
	protected void actionPerformedBtnFind(ActionEvent e) {
		...
		if (com == "전체보기") {
			...
			list = service.showOdTableReleaseDatNo(no);
			if (list == null) {
			...
			list = service.showOdTableReleaseCNo(cno);
			if (list == null) {
				throw new NullException();
			}
			pTable.setSearchList(list);
			...
			intCheck();
			String cName = tf;
			list = service.showOdTableReleaseCName(cName);
			if (list == null) {
				throw new NullException();
			}
			pTable.setSearchList(list);
			...
			validCheck();
			String code = tf;
			list = service.showOdTableReleaseCode(code);
			if (list == null) {
				throw new NullException();
			}
			pTable.setSearchList(list);
			...
			validCheck();
			String product = tf;
			list = service.showOdTableReleaseProduct(product);
			if (list == null) {
				throw new NullException();
			}
			pTable.setSearchList(list);
